...
station_ave <- met[, .(temp = mean(temp, na.rm = T), wind.sp = mean(wind.sp, na.rm = T), atm.press = mean(atm.press, na.rm = T)), by=.(USAFID, STATE)]
head(station_ave)
```
```{r}
medians <- station_ave[, .(temp_50 = quantile(temp, probs=0.5, na.rm=T), wind.sp_50 = quantile(wind.sp, probs=0.5, na.rm=T), atm.press_50 = quantile(atm.press, probs=0.5, na.rm=T))]
medians
```
...
```{r}
mid_point <- met[, .(lon_50 = quantile(lon, probs=.5, na.rm=T), lat_50 = quantile(lat, probs=.5, na.rm=T)), by=STATE]
...
library(leaflet)
leaflet() %>% addProviderTiles('CartoDB.Positron') %>% addCircles(data = mid_station, lat=~lat, lng=~lon, popup="mid station", opacity=1, fillOpacity=1, radius=400, color="Blue")
```
...
tab <- met[, .(N_entries = .N, N_entries_na = sum(is.na(temp_cat)), N_stations = length(unique(USAFID)), N_states = length(unique(STATE)), mean_temp = mean(temp, na.rm=T), mean_wind.sp = mean(wind.sp, na.rm=T), mean_atm.press = mean(atm.press, na.rm=T)), by=temp_cat]

knitr::kable(tab, caption="Information computed")
```
...
-   https://github.com/Fredooooooo/JSC370
